...
export default function Home() {
  ...
  const fetchBalance = async () => {
    try {
      ...
      const balanceResponse = await publicClient?.getBalance({ address: formattedAddress as `0x${string}` });
  ...
  const executeTxEthers = async () => {
    const tx = {
      ...
      data: "0x" as const, // No data, as there is no contract interaction
    };
...
//         account: primaryWallet.accounts[0] as `0x${string}`, // Primary account
//       };
