...
__export(src_exports, {
  ...
  getSolanaChains: () => getSolanaChains,
...
var threshSig = {
  async edDSA(data) {
    const module2 = await import("@particle-network/thresh-sig");
    const { EdDsa } = module2.default || module2;
    return EdDsa.from(data);
  },
  async ecDSA(data) {
    const module2 = await import("@particle-network/thresh-sig");
    const { Ecdsa } = module2.default || module2;
  ...
  async decryptData(ciphertext, key) {
    try {
      const module2 = await import("@particle-network/thresh-sig");
      const { decryptData } = module2.default || module2;
  ...
  async encryptData(plaintext, key) {
    const module2 = await import("@particle-network/thresh-sig");
    const { encryptData } = module2.default || module2;
    const encrypted = await encryptData(plaintext, key);
    return encrypted;
  },
  async generateKey(password, uuid5) {
    const module2 = await import("@particle-network/thresh-sig");
    const { generateKey } = module2.default || module2;
      ...
      if (encryptedKMSDataKey) {
        ...
        const [{ p2Key, address }, kmsDataKey] = await Promise.all([
          ...
          generateDataKey().then(({ ciphertext, plaintext }) => {
            return Promise.all([threshSig_default.generateKey(plaintext, userInfo.uuid), Promise.resolve(ciphertext)]);
          })
        ]);
        p2KeyValue = p2Key;
        publicAddress = address;
...
var connect = async (param) => {
  ...
  if ("phone" in param && "code" in param && param.phone && param.code) {
    ...
    userInfo = await pnApi_default.loginWithThirdparty({ provider: param.provider, thirdpartyCode: param.thirdpartyCode });
    latestAuthType = param.provider;
  ...
  try {
    await wallet_default.gen(
      chainInfo ? getChainType(chainInfo) === "evm" ? "evm_chain" /* EVM */ : "solana" /* Solana */ : "evm_chain" /* EVM */
    );
  } catch (error) {
...
var EthereumProvider = class {
  ...
  async request(data) {
    ...
    if (Object.values(EvmRpcMethod).includes(method)) {
      if (method === "eth_accounts" /* ethAccounts */) {
        if (this.selectedAddress) {
          return [this.selectedAddress];
        }
        return [];
      } else if (method === "eth_requestAccounts" /* ethRequestAccounts */) {
  ...
  async sendTransaction(transaction) {
    ...
    if (isNullish(transaction.type)) {
      const chain2 = getEVMChainInfoById(chainId);
      if (chain2 && isChainSupportEIP1559(transaction)) {
    ...
    if (isTron(chainId)) {
      ...
      const module2 = await import("@ethereumjs/tx");
      const { TransactionFactory } = module2.default || module2;
...
// src/core/solanaWallet.ts
var _events2, _connecting, _chainId2, _genWallet2, genWallet_fn2, _sendAnalyticsActive2, sendAnalyticsActive_fn2;
var SolanaWallet = class {
  constructor() {
    ...
    eventUtils_default.on("connectWithChain", (viemChain) => {
      if (getChainType(viemChain) === "solana") {
        __privateSet(this, _chainId2, viemChain.id);
        setCurrentSolanaChainId(__privateGet(this, _chainId2));
  ...
  async loadPublicKey() {
    if (this.selectedAddress && !this.publicKey) {
      ...
      const { PublicKey } = module2.default || module2;
  ...
  async signAndSendTransaction(transaction, chainId) {
    ...
    const signature = await solanaApi_default.request({
...
var _AuthCore = class {
  constructor() {
    ...
    this.chains = [import_chains.mainnet];
  }
  static getInstance() {
